# -*- coding: utf-8 -*-
"""
Created on Wed Jun 18 16:10:10 2025
...
data_file = (r"C:\Users\parai\Downloads\wetfoam2_bub_RH2_0.020000_0.507713.dat")
num_steps = 726
...
def plot_bubble_trajectory(bubble_id):
    if bubble_id in x_coords:
        ...
        x_data = x_coords[bubble_id]
        y_data = y_coords[bubble_id]
        
        # Plot the trajectory
        plt.plot(x_data, y_data, label='Lifetime = ' + str(lifetime), marker=',')
        
        # Add start point marker (green circle)
        plt.plot(x_data[0], y_data[0], 'go', markersize=8, markeredgecolor='darkgreen', 
                markeredgewidth=1, label='Start' if bubble_id == 1 else "")
        
        # Add end point marker (red circle)  
        plt.plot(x_data[-1], y_data[-1], 'ro', markersize=8, markeredgecolor='darkred',
                markeredgewidth=1, label='End' if bubble_id == 1 else "")

def plot_unwrapped_trajectory(bubble_id):
    if bubble_id in x_unwrapped:
        lifetime = len(x_unwrapped[bubble_id])
        x_data = x_unwrapped[bubble_id]
        y_data = y_unwrapped[bubble_id]
...
# Example usage for plotting trajectories with start/end markers
# Uncomment the sections below to use:

#Plot all Bubble Trajectories with start/end markers
'''
plt.figure(figsize=(12, 10), dpi=300)      
plot_n_raw(10)  # Plot first 10 bubbles as example
plt.tight_layout()
plt.title('Raw Bubble Trajectories with Start/End Points')
plt.xlabel('X Position')
plt.ylabel('Y Position')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig(f'{MSD_FOLDER}/raw_trajectories_with_markers.pdf')
plt.show() 
'''

# Uncomment to plot unwrapped trajectories

plt.figure(figsize=(12, 10), dpi=300)
for i in range(1, 400):  # Plot first n unwrapped trajectories
    plot_unwrapped_trajectory(i)
plt.tight_layout()
plt.title('Unwrapped Bubble Trajectories with Start/End Points')
plt.xlabel('X Position (Unwrapped)')
plt.ylabel('Y Position (Unwrapped)')
#plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('Bubble_Trajectories_definitive_507713.pdf')
plt.show()
